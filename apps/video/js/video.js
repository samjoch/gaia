...
function createThumbnailItem(videonum) {
  ...
  details.addEventListener('underflow', detailsUnderflowHandler);
...
function detailsUnderflowHandler(e) {
  var el = e.target;
  el.dataset.counter = 1;
  el.firstChild.textContent = el.dataset.title;
}

function detailsOverflowHandler(e) {
  var el = e.target;
  var dataset = el.dataset;
  el.firstChild.textContent = dataset.title.slice(0,-dataset.counter);
  if (-dataset.counter !== dataset.title.length) {
...
window.addEventListener('resize', function() {
  ...
  for (var i = 0; i < texts.length; i++) {
    texts[i].dataset.counter = -texts[i].dataset.title.length;
    detailsOverflowHandler({ type: 'resize', target: texts[i] });
  }
